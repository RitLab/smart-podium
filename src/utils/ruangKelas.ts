 ...
 * paling enak, tapi respons daftar ruang emang belum punya penanda apa pun soal
...
// daripada ikut-ikutan salah — "class_name" udah dipakai respons API buat arti
// yang beda (nama rombel, bukan nama ruang).
 ...
 * tapi "Ruang Maulana" sama "Paula" nggak — padahal ngandung "aula".
 */
...
/**
 * Namanya disimpan BARENG id pemiliknya, dan cuma dipakai kalau id-nya masih
 * cocok sama class_id yang berlaku sekarang.
 *
 * Ini bukan kehati-hatian berlebihan. class_id ditulis dari lebih dari satu
 * tempat — selain halaman pilih kelas, halaman lisensi juga nulis langsung
 * (src/pages/Auth/LicenseKey.tsx). Kalau namanya disimpan sendirian, ganti ruang
 * lewat jalur yang nggak ikut nulis nama bakal ninggalin nama ruang LAMA, dan
 * menunya salah sembunyi. Diikat ke id begini, basi jadi nggak mungkin: id
 * nggak cocok, namanya dianggap nggak ada.
 */
export const namaRuangTersimpan = (): string => {
  const mentah = localStorage.getItem(KUNCI_NAMA);
  if (!mentah) return "";
  try {
    const isi = JSON.parse(mentah) as { id?: string; nama?: string };
    if (!isi?.nama || !isi?.id) return "";
    return isi.id === idRuangTersimpan() ? isi.nama : "";
  } catch {
    // Bentuk lama atau ketimpa sembarangan — jangan dipercaya.
    return "";
  }
};

const tulisNama = (id: string, nama: string) => {
  if (!id || !nama) {
    localStorage.removeItem(KUNCI_NAMA);
    return;
  }
  localStorage.setItem(KUNCI_NAMA, JSON.stringify({ id, nama }));
};

/**
 * Dipanggil pas guru milih ruang. Namanya ikut disimpan karena sesudah ini
 * daftar ruang nggak pernah diambil lagi — tanpa ini, begitu app di-restart
...
export const simpanNamaRuang = (nama: string) => {
  tulisNama(idRuangTersimpan(), nama);
};
